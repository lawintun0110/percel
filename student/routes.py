...
def certification(enrollment_id):
    ...
    base_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    certificate_json_path = os.path.join(base_path,certificate_file_name)
    score_json_path = os.path.join(base_path,file_name)
    chapter_name_dict = {}
    if  os.path.exists(certificate_json_path) and os.path.exists(score_json_path):
        with open(certificate_json_path, "r") as f :
            certificate_data = json.load(f)
            certificated_data = certificate_data[str(student_id)]
        with open(score_json_path, "r") as f:
            score_data = json.load(f)
            scored_data = score_data[str(student_id)]
            if learning_curve:
                ...
                for ch in chapter:
                      if ch.id in chapter_list:
                        ...
                        if certificated_data[str(ch.id)] and scored_data[str(ch.id)]:
                            if certificated_data[str(ch.id)] == 1 and scored_data[str(ch.id)] == 1:
